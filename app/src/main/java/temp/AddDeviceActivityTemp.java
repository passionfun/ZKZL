...
import com.sunparlcompany.zkel.ui.UnBindDeviceListActivity;
...
public class AddDeviceActivityTemp extends AppCompatActivity implements View.OnClickListener{
    private static final String tag = AddDeviceActivityTemp.class.getSimpleName();
    ...
    private void storeDeviceInDb(String deviceInfo) {
        ...
        try {
            ...
            startActivity(new Intent(AddDeviceActivityTemp.this,UnBindDeviceListActivity.class));
            finish();
        } catch (DbException e) {
            e.printStackTrace();
        }
    ...
    private void startSearchDevice() {
        micodev.startSearchDevices(serviceName, new SearchDeviceCallBack() {
            ...
            public void onDevicesFind(int code, JSONArray deviceStatus) {
                Log.i(tag,"onDeviceFind:"+deviceStatus.toString());
                Message msg = Message.obtain();
...
//                }
    ...
    private class MyHandler extends Handler {
        private  WeakReference<AddDeviceActivityTemp> mWeakReference;

        public MyHandler(AddDeviceActivityTemp activity) {
            this.mWeakReference = new WeakReference<AddDeviceActivityTemp>(activity);
        }

        @Override
        public void handleMessage(Message msg) {
            AddDeviceActivityTemp addDeviceActivity = mWeakReference.get();
...
//                    showToast("停止搜索设备失败");
//                    break;
                case STOP_FIND_DEVICE_SUCCESS://停止搜索设备成功
                    Log.i(tag,"停止搜索设备成功");
//                    showToast("停止搜索设备成功");
                    break;
                default:break;
